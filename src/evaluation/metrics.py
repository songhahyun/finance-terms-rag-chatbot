...
def mrr_score(retrieved_ids: Iterable[str], golden_ids: Iterable[str]) -> float:
    """Compute reciprocal rank of the first relevant retrieved chunk."""
    golden = set(golden_ids)
    if not golden:
        return 0.0
...
def measure_retrieval_latency(
    ...
    **kwargs,
    ...
    """Run a retrieval callable and return (result, latency_sec)."""
    start = perf_counter()
    result = retrieve_fn(*args, **kwargs)
    latency_sec = perf_counter() - start
    return result, latency_sec
